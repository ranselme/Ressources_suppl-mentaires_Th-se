...
Le motif « o » a une durée moyenne de `r round(mean(dplyr::filter(table_burst,elements_full=="o")$durat)*1000,2)`ms et une médiane de `r round(median(dplyr::filter(table_burst,elements_full=="o")$durat)*1000,2)`ms (maximum à `r round(max(dplyr::filter(table_burst,elements_full=="o")$durat)*1000,2)`ms, minimum à `r round(min(dplyr::filter(table_burst,elements_full=="o")$durat)*1000,2)`ms et IQR de `r round(IQR(dplyr::filter(table_burst,elements_full=="o")$durat)*1000,2)`ms).
Sa durée moyenne est inférieure à celle de tous les autres motifs (p<0.001 dans les neuf cas).
Le motif « a » a une durée moyenne de `r round(mean(dplyr::filter(table_burst,elements_full=="a")$durat)*1000,2)`ms et une médiane de `r round(median(dplyr::filter(table_burst,elements_full=="a")$durat)*1000,2)`ms (maximum à `r round(max(dplyr::filter(table_burst,elements_full=="a")$durat)*1000,2)`ms, minimum à `r round(min(dplyr::filter(table_burst,elements_full=="a")$durat)*1000,2)`ms et IQR de `r round(IQR(dplyr::filter(table_burst,elements_full=="a")$durat)*1000,2)`ms).
...
```{r fig.cap=capFig("Durées moyennes des « o » en fonction des motifs les contenant. Aucune différence n’est faite entre le premier « o » d’un motif ou le deuxième « o », bien que la durée de l’élément « o » diminue lorsqu’il est placé plus à droite dans le motif."), fig.width=12, fig.height=7}
...
Durées moyennes des « o » en fonction des motifs les contenant. Aucune différence n’est faite entre le premier « o » d’un motif ou le deuxième « o », bien que la durée de l’élément « o » diminue lorsqu’il est placé plus à droite dans le motif.
...
Un test de Kruskal–Wallis permet de mettre en évidence que les éléments « o » ont une durée différente en fonction du motif dans lequel ils sont inclus (H(`r test_kruskal_duree_o[[4]]`)=`r test_kruskal_duree_o[[3]]`, p=`r test_kruskal_duree_o[[5]]`).
...
```{r fig.cap=capFig("Fréquences des différents motifs en fonction des contextes. Les fréquences sont calculées par ligne. Une case jaune signifie qu’un motif est intégralement trouvé dans un seul contexte, une case violette signifie qu’un motif n’est pas présent dans ce contexte."), fig.width=8, fig.height=7}
table_burst %>% 
...
Fréquences des différents motifs en fonction des contextes. Les fréquences sont calculées par ligne. Une case jaune signifie qu’un motif est intégralement trouvé dans un seul contexte, une case violette signifie qu’un motif n’est pas présent dans ce contexte.
...
Pour le contexte intervocalique, indépendamment du motif, on retrouve une multitude de voyelles avant le motif.
Le [a] représente `r round(context_VrV_left[context_VrV_left$.=="a",]$Freq_pc,2)`% des occurrences, suivi du [e] (`r round(context_VrV_left[context_VrV_left$.=="e",]$Freq_pc,2)`%) du [ə] (`r round(context_VrV_left[context_VrV_left$.=="ə",]$Freq_pc,2)`%) et du [i] (`r round(context_VrV_left[context_VrV_left$.=="i",]$Freq_pc,2)`%).
Après les motifs, on retrouve le [a] qui représente `r round(context_VrV_right[context_VrV_right$.=="a",]$Freq_pc,2)`% des occurrences, le [i] `r round(context_VrV_right[context_VrV_right$.=="i",]$Freq_pc,2)`% des occurrences et le [o] et le [u] représentent chacun `r round(context_VrV_right[context_VrV_right$.=="o",]$Freq_pc,1)`%.
