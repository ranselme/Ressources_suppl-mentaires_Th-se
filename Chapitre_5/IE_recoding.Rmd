...
L’analyse originelle comportait `r sum(table_changes$Freq)` langues. De ces langues nous n’en avons gardé que `r sum(table_changes[table_changes$revision=="other",]$Freq) + sum(table_changes[table_changes$revision=="trilled",]$Freq)` pour `r nrow(unique(dplyr::filter(rough_r_data, !is.na(revision) & revision=="trilled" | revision=="other") %>%  dplyr::select(Family)))` familles de langues après notre recodage.
